...
# 一个完整的高级检索url示例
# https://s.weibo.com/weibo?q=计算机等级考试&typeall=1&suball=1&timescope=custom:2021-04-12-0:2021-04-13-0&Refer=g
# 上面url表示搜索从2021-04-12的0点到2021-04-13的0点，这期间有关计算机等级考试的微博，时间跨度为04-12一整天
class SearchSpider(scrapy.Spider):
    name = 'search'
    allowed_domains = ['weibo.com']
    settings = get_project_settings()  # 获取settings.py里的配置信息
    keyword_list = settings.get('KEYWORD_LIST')

    weibo_type = util.convert_weibo_type(settings.get('WEIBO_TYPE'))  # 格式举例：'&typeall=1'
    contain_type = util.convert_contain_type(settings.get('CONTAIN_TYPE'))  # 筛选结果微博中必需包含的内容，格式举例：'&suball=1'
    regions = util.get_regions(settings.get('REGION'))
    base_url = 'https://s.weibo.com'
    start_date = settings.get('START_DATE',datetime.now().strftime('%Y-%m-%d'))  # 格式举例：'2020-01-23'
    end_date = settings.get('END_DATE', datetime.now().strftime('%Y-%m-%d'))  # 格式举例：'2020-01-25'
    mongo_error = False
    pymongo_error = False
    mysql_error = False
    pymysql_error = False
    ...
    def start_requests(self):
        start_date = datetime.strptime(self.start_date, '%Y-%m-%d')  # 格式举例：2020-01-23 00:00:00
        end_date = datetime.strptime(self.end_date, '%Y-%m-%d') + timedelta(days=1)  # 格式举例：2020-01-26 00:00:00
        start_str = start_date.strftime('%Y-%m-%d') + '-0'  # 格式举例：'2020-01-23-0'
        end_str = end_date.strftime('%Y-%m-%d') + '-0'  # 格式举例：'2020-01-26-0'
        for keyword in self.keyword_list:
            # 当对地区的搜索条件为全部或未设置地区搜索条件时，进入这个判断
    ...
    def parse(self, response):
        ...
        # 当未搜索到任何博文时，页面中会生成这个元素，提示：抱歉，未找到“搜索关键字”相关结果...
    ...
    #     """解析网页中的微博信息"""
    #     keyword = response.meta.get('keyword')
    #     for sel in response.xpath("//div[@class='card-wrap']"):  # //div[@class='card-wrap']每一条微博的最外层的容器，数据在该容器内部逐项渲染
    #         info = sel.xpath(
    #             "div[@class='card']/div[@class='card-feed']/div[@class='content']/div[@class='info']"
    #         )
    #         if info:
